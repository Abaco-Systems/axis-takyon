...
static void *thread_entry_function(void *user_data) {
  ...
  // Run correct thread
  pipelineTask(L_graph, thread_info->group_id, L_ncycles);
...
#ifdef VXWORKS_7
int pipeline(int process_id_arg, char *graph_filename_arg, int ncycles_arg) {
  if (graph_filename_arg == NULL) {
    printf("Usage: pipeline(<process_id>,\"<graph_filename>\",<ncycles>)\n");
    return 1;
  }

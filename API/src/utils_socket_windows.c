...
static bool socket_send_event_driven(TakyonSocket socket, void *addr, size_t total_bytes_to_write, int64_t timeout_ns, bool *timed_out_ret, char *error_message) {
  ...
  if (setsockopt(socket, SOL_SOCKET, SO_SNDTIMEO, (char *)&timeout_ms, sizeof(timeout_ms)) != 0) {
...
static bool socket_recv_event_driven(TakyonSocket socket_fd, void *data_ptr, size_t bytes_to_read, int64_t timeout_ns, bool *timed_out_ret, char *error_message) {
  ...
  if (setsockopt(socket_fd, SOL_SOCKET, SO_RCVTIMEO, (char *)&timeout_ms, sizeof(timeout_ms)) != 0) {
    TAKYON_RECORD_ERROR(error_message, "Failed to set the timeout = %lld. sock_error=%d\n", timeout_ns, WSAGetLastError());
    return false;
  }
...
bool socketCreateTcpServer(const char *ip_addr, uint16_t port_number, TakyonSocket *socket_fd_ret, int64_t timeout_ns, char *error_message) {
  if (!windows_socket_manager_init(error_message)) {
    return false;
  }
